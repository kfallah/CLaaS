...
- For the **local** profile: NVIDIA GPU with >= 24 GB VRAM and the [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html)
- For the **tinker** profile: A Tinker API key
...
### Tinker profile (no GPU)

```bash
cd docker
cp .env.example .env
# Edit .env: set TELEGRAM_BOT_TOKEN and TINKER_API_KEY
docker compose --profile tinker up --build
```
...
**Telegram bot doesn't respond**: Verify `TELEGRAM_BOT_TOKEN` is set and the bot has been started in Telegram (send `/start`). Check logs: `docker compose --profile local logs -f openclaw-local` (or `--profile tinker logs -f openclaw-tinker`).
