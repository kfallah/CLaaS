...
from claas.eval.config import build_harness_config, load_config
...
def test_load_config_with_overrides():
    ...
    config = load_config(overrides=["num_steps=5", "seed=99"])
...
def test_tinker_proxy_default():
    ...
    config = load_config(overrides=["mode=tinker", "proxy_url=null"])
...
def test_timestamped_output_dir():
    ...
    config = load_config(overrides=["output_dir=/tmp/test-evals"])
...
def test_collapse_steps_list():
    ...
    config = load_config(overrides=["collapse_steps=[0,5,10]"])
...
def test_steps_per_batch_from_config():
    ...
    config = load_config(overrides=["steps_per_batch=3"])
...
def test_load_config_custom_dir():
