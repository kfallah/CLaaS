...
def test_dashboard_page2_shows_older_record(monkeypatch, tmp_path):
    """Page 2 shows the older record when per_page=1."""
    _mock_config(monkeypatch, "local", feedback_log_dir=str(tmp_path))
        ...
        """
{
  ...
  "vllm": {"slept": true, "woke": true},
  "timing_ms": {"sleep": 1, "distill": 2, "save": 0, "wake": 1, "logprobs": 0, "total": 4},
  "distill_result": {"lora_id": "user/model", "metadata": {"loss": 0.1}},
  "error": null
}
""".strip(),
        encoding="utf-8",
    ...
    (tmp_path / "20240101T000002-b.json").write_text(
        (tmp_path / "20240101T000001-a.json")
        .read_text(encoding="utf-8")
        .replace('"a"', '"b"', 1)
        .replace("prompt-a", "prompt-b"),
        encoding="utf-8",
    )

    client = TestClient(web_app)
    response = client.get("/v1/dashboard", params={"page": 2, "per_page": 1})

    assert response.status_code == 200
    assert "prompt-a" in response.text
    assert "prompt-b" not in response.text
    assert "Page 2 of 2" in response.text


def test_dashboard_page_clamped_to_last(monkeypatch, tmp_path):
...
""".strip(),
    ...
    client = TestClient(web_app)
    response = client.get("/v1/dashboard", params={"page": 999, "per_page": 20})
