...
# ── Token ID propagation tests ──────────────────────────────────────


def test_engine_distill_uses_response_token_ids(tinker_engine, mock_training_client):
    """When response_token_ids is set, the engine uses them directly (no re-tokenization)."""
    engine, mock_service = tinker_engine
    ...
    # Provide explicit token IDs — the tokenizer should NOT be called for
    ...
    assert isinstance(result, DistillResponse)
    assert result.metadata["step"] == 1
    # completion_len should match response_token_ids length (3), not text length
    assert result.metadata["completion_len"] == 3

    # Tokenizer.encode should NOT have been called for prompt or response
    tok = mock_training_client.get_tokenizer()
    encode_calls = [
        c for c in tok.encode.call_args_list
        if "(ignored" in str(c)
    ]
...
def test_engine_distill_uses_user_prompt_for_teacher(tinker_engine, mock_training_client):
    ...
    with patch(
        "claas.training.engine.tinker.engine.build_teacher_messages"
    ) as mock_build:
    ...
    # build_teacher_messages should receive the clean user_prompt, not the
    # template-decorated prompt
