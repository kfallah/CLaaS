"""Regression tests for optional environment-variable fallbacks."""
...
def _install_torch_stub(monkeypatch) -> None:
    torch_stub = cast(Any, types.ModuleType("torch"))
    ...
    nn_module = cast(Any, types.ModuleType("torch.nn"))
    nn_functional_module = cast(Any, types.ModuleType("torch.nn.functional"))
...
def test_distillation_uses_transformers_cache_fallback(monkeypatch) -> None:
    monkeypatch.delenv("HF_HOME", raising=False)
    monkeypatch.setenv("TRANSFORMERS_CACHE", "/tmp/transformers-cache")

    recorded: dict[str, str | None] = {}

    transformers_stub = cast(Any, types.ModuleType("transformers"))
    ...
    distillation_module = _reload("claas.training.distillation")
    trainer = distillation_module.DistillationTrainer(
