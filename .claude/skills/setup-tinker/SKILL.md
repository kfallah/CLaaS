...
```bash
docker compose --env-file docker/.env.tinker --profile tinker up --build -d
```
...
```bash
docker compose --env-file docker/.env.tinker --profile tinker logs -f init-tinker 2>&1 | head -50
```
...
docker compose --env-file docker/.env.tinker --profile tinker logs openclaw-tinker 2>&1 | tail -20
```
...
- **Plugin ECONNREFUSED on port 8080**: The OpenClaw feedback plugin must reach `http://claas-api:8080` (Docker service DNS), not `localhost`. Re-run the init container: `docker compose --env-file docker/.env.tinker --profile tinker up init-tinker`.
- **Containers keep restarting**: Check logs with `docker compose --env-file docker/.env.tinker --profile tinker logs -f <service>`.
