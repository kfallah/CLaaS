...
| `rollout_logprobs` | null | Logprobs from inference server for off-policy IS correction |
...
For proper off-policy learning (when the response was generated by a different model checkpoint), pass the `rollout_logprobs` field with the log-probabilities from the inference server that generated the rollout:
